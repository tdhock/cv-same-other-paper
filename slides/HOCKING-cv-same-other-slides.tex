...
\begin{frame}[fragile]
  \frametitle{Same Other CV for MNIST data}
...
\begin{frame}[fragile]
  \frametitle{Same Other CV for spam data}
...
\begin{frame}[fragile]
  \frametitle{Same Other CV for vowel data}
...
\begin{frame}[fragile]
  \frametitle{Same Other CV for KMNIST data}
...
\begin{frame}
  ...
  \begin{itemize}
  \item Proposed Same Other Cross-Validation can be used to see if
    data sets are similar enough to so that combining data is
    beneficial for training (train on one group, test/predict on
    another).
  \item In Autism data, there was a slight benefit to combining years.
  \item In image classification data (fires/trees/fish), we observed
