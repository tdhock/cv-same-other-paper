...
fr <- function(DT){
  DT[, let(
    Données=Data,
    "sous-ensemble test"=ifelse(predefined.set=="test","C","D")
  )]
  if("algorithm" %in% names(DT))DT[, let(
    Algorithme=factor(algorithm, algo.info$algorithm, algo.info$Algorithme)
  )]
  if("train_subsets" %in% names(DT))DT[, let(
    entraînement=factor(train_subsets, rev(yinfo$en), rev(yinfo$fr))
  )]
}
fr(show.both)
fr(show.meta)
...
gg <- ggplot()+
  ...
  scale_color_manual(
    "Algorithme",
    values=algo.info[, structure(color, names=Algorithme)])+
  geom_text(aes(
    Inf, Inf, label=sprintf("%d lignes ", value)),
    hjust=1,
    vjust=1.2,
    size=text.size,
    color="grey50",
    data=show.meta)+
  geom_segment(aes(
    compare_mean, entraînement,
    color=Algorithme,
    xend=same_mean, yend=entraînement),
    linewidth=1,
    data=show.p)+
  geom_text(aes(
  ...
  geom_segment(aes(
    ...
    data=show.both)+
  geom_point(aes(
    percent.error_mean, entraînement, color=Algorithme),
    shape=1,
    data=show.both)+
  geom_text(aes(
    percent.error_median, entraînement,
    color=Algorithme,
